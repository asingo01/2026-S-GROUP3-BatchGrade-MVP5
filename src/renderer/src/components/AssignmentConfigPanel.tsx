                ...
                await window.api.assignments.update({
                    ...
                    solutionFilePath:
                        form.solutionType === 'file' && selectedFile
                            ...
                            : undefined,
                    expectedOutputText:
                        form.solutionType === 'text' ? trimmedExpectedOutput : undefined
                })
            } else {
                await window.api.assignments.create({
                    ...
                    solutionFilePath:
                        form.solutionType === 'file' && selectedFile
                            ...
                            : null,
                    expectedOutputText:
                        form.solutionType === 'text' ? trimmedExpectedOutput : null,
        ...
        <div className="panel-shell">
            ...
            <div className="panel-form">
                ...
                <textarea
                    ...
                    onChange={(e) =>
                        setForm((f) => ({ ...f, gradingCriteria: e.target.value }))
                    }
                    className="panel-input"
                    rows={4}
                />
                ...
                {form.solutionType === 'text' ? (
                    <textarea
                        ...
                        onChange={(e) =>
                            setForm((f) => ({ ...f, expectedOutputText: e.target.value }))
                        }
                        className="panel-input"
                        rows={5}
                    />
                    ...
                    <div className="panel-section">
                        ...
                        />
                        {selectedFile && (
                            <p className="text-xs">selected file: {selectedFile.name}</p>
                        )}
                    </div>
                )}
                    ...
                    {assignments.map((assignment) => (
                        <li key={assignment.uuid} className="panel-list-item">
                            <div>
                                ...
                                {assignment.solutionType === 'text' &&
                                    assignment.expectedOutputText && (
                                        <div>expected output: {assignment.expectedOutputText}</div>
                                    )}

                                {assignment.solutionType === 'file' &&
                                    assignment.solutionFileName && (
                                        <div>solution file: {assignment.solutionFileName}</div>
                                    )}

                                <div>
                                    created:{' '}
                                    {new Date(assignment.createdAt * 1000).toLocaleString()}
                                </div>
